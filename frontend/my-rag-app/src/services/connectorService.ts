...
export interface Connector { id: string; document_set_id: string; connector_type: "website" | "github"; name: string; source_url: string; status: string; last_error: string | null; last_synced_at: string | null; schedule_enabled: boolean; schedule_interval: "hourly" | "daily" | "weekly"; next_sync_at: string | null; created_at: string; }
export const connectorService = {
  list: (setId: string) => request<Connector[]>(`/document-sets/${setId}/connectors`, { headers: headers() }),
  create: (setId: string, payload: { connector_type: "website" | "github"; name: string; source_url: string; schedule_enabled?: boolean; schedule_interval?: "hourly" | "daily" | "weekly" }) => request<Connector>(`/document-sets/${setId}/connectors`, { method: "POST", headers: headers(true), body: JSON.stringify(payload) }),
  sync: (setId: string, id: string) => request<{ discovered: number; created: number; updated: number; unchanged: number }>(`/document-sets/${setId}/connectors/${id}/sync`, { method: "POST", headers: headers() }),
