...
export default function AssistantsPage() {
  ...
  const ask = async (content: string) => { if (!selected) return false; setMessages((m) => [...m, { id: crypto.randomUUID(), role: "user", content, createdAt: new Date().toISOString() }]); setThinking(true); try { const result = await assistantService.ask(selected.id, content); setMessages((m) => [...m, { id: crypto.randomUUID(), role: "assistant", content: result.answer, responseId: result.response_id, grounded: result.grounded, answerBasis: result.answer_basis, createdAt: new Date().toISOString(), sources: result.citations.map((x) => ({ id: x.chunk_id, citationId: x.id, documentId: x.document_id, title: x.filename, chunkIndex: x.chunk_index, excerpt: x.excerpt, score: x.score, page: x.page, section: x.section })) }]); return true; } catch (e) { toast.error((e as Error).message); return false; } finally { setThinking(false); } };
