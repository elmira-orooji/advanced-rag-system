...
import { CircleAlert, ChevronDown, Plus, RefreshCw, Search, Trash2 } from "lucide-react";
...
export default function UsersPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    userService.list().then((items) => { if (!cancelled) { setUsers(items); setLoadError(null); } })
      .catch((error: Error) => { if (!cancelled) { setLoadError(error.message); toast.error(error.message); } })
