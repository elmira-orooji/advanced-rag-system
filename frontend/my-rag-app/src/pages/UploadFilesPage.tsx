...
function CloudConnectorDialog(props: { setId: string; isFa: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  if (webhookInfo) return <div className="fixed inset-0 z-[85] grid place-items-center bg-black/75 p-4 backdrop-blur-md"><div className="app-glass-panel w-full max-w-xl rounded-[24px] border border-white/10 p-6"><span className="grid size-10 place-items-center rounded-xl bg-[#32127A]/30 text-[#b6a7ef]"><Zap size={18} /></span><h2 className="mt-4 text-xl font-semibold">{isFa ? "Webhook آماده است" : "Webhook is ready"}</h2><p className="mt-2 text-xs leading-5 text-amber-100/45">{isFa ? "Secret فقط همین یک‌بار نمایش داده می‌شود؛ اکنون آن را ذخیره کنید." : "The secret is shown only once. Store it securely now."}</p>{([['Endpoint', webhookInfo.endpoint], ['X-Webhook-Secret', webhookInfo.secret]] as const).map(([label, value]) => <div key={label} className="mt-4"><p className="mb-2 text-[10px] font-semibold text-white/40">{label}</p><div className="flex items-center gap-2 rounded-xl border border-white/[.08] bg-black/25 p-2"><code className="min-w-0 flex-1 overflow-x-auto px-2 text-[10px] text-white/60">{value}</code><button type="button" onClick={() => { void navigator.clipboard.writeText(value); toast.success("Copied"); }} className="rounded-lg bg-white/[.06] px-3 py-2 text-[10px] text-white/55">Copy</button></div></div>)}<button type="button" onClick={onSaved} className="mt-6 w-full rounded-xl bg-[#32127A] px-5 py-3 text-xs font-semibold">{isFa ? "انجام شد" : "Done"}</button></div></div>;
  return <div className="fixed inset-0 z-[85] grid place-items-center bg-black/75 p-4 backdrop-blur-md" onMouseDown={onClose}><motion.form initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} onSubmit={submit} onMouseDown={(event) => event.stopPropagation()} className="app-glass-panel max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-[24px] border border-white/10 p-6"><div className="flex items-start justify-between"><div><span className="grid size-10 place-items-center rounded-xl bg-[#32127A]/30 text-[#b6a7ef]"><Link2 size={18} /></span><h2 className="mt-4 text-xl font-semibold">{isFa ? "اتصال منبع جدید" : "Connect a new source"}</h2><p className="mt-2 text-xs text-white/30">{isFa ? "منبع ابری یا عمومی را به این پایگاه دانش متصل کنید." : "Connect a cloud or public source to this knowledge base."}</p></div><button type="button" onClick={onClose} className="app-icon-button grid size-9 place-items-center rounded-xl text-white/40"><X size={16} /></button></div><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3">{options.map((option) => <button type="button" key={option.type} onClick={() => { setType(option.type); setUrl(""); }} className={`flex items-center gap-2 rounded-xl border p-3 text-xs transition ${type === option.type ? "border-[#8f78d8]/35 bg-[#32127A]/25 text-white/80" : "border-white/[.08] text-white/35 hover:bg-white/[.035]"}`}>{option.icon}{option.label}</button>)}</div><label className="mt-5 block text-xs font-semibold text-white/50">{isFa ? "نام اتصال" : "Connection name"}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={options.find((option) => option.type === type)?.label} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm outline-none placeholder:text-white/20 focus:border-[#8f78d8]/50" /></label><label className="mt-4 block text-xs font-semibold text-white/50">URL<input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder={placeholders[type]} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm outline-none placeholder:text-white/20 focus:border-[#8f78d8]/50" /></label><div className="mt-4 rounded-xl border border-white/[.07] bg-black/20 p-3 text-[10px] leading-5 text-white/35">{type === "google_drive" ? "Google OAuth credentials and a folder URL are required." : type === "s3" ? "Use an AWS S3 HTTPS bucket/prefix URL; credentials stay on the server." : type === "sharepoint" ? "Use the Microsoft Graph /children URL for the target drive folder." : "Only public HTTPS sources are supported."}</div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-xs text-white/40">{isFa ? "انصراف" : "Cancel"}</button><button disabled={!valid || saving} className="rounded-xl bg-[#32127A] px-5 py-2.5 text-xs font-semibold disabled:opacity-40">{saving ? (isFa ? "در حال همگام‌سازی…" : "Syncing…") : isFa ? "اتصال و همگام‌سازی" : "Connect and sync"}</button></div></motion.form></div>;
}
...
function ChunkNumber({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) {
  return <label className="text-[10px] font-semibold text-white/45">{label}<input type="number" value={value} min={min} max={max} onChange={(event) => onChange(Number(event.target.value))} className="mt-2 h-11 w-full rounded-xl border border-white/[.09] bg-black/25 px-3 text-sm text-white/70 outline-none focus:border-[#8f78d8]/45" /><span className="mt-1 block text-[8px] font-normal text-white/20">{min} – {max} chars</span></label>;
}
