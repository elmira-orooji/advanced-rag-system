    ...
    <main className="min-w-0 flex-1 overflow-hidden px-4 py-5 sm:px-6 lg:px-8">
      <div className="mx-auto flex h-full w-full max-w-[980px] flex-col gap-5">
        <header className="flex shrink-0 items-end justify-between gap-4">
          <div><div className="mb-2 flex items-center gap-2 text-[11px] font-bold uppercase tracking-[.2em] text-[#c43cff]"><span className="size-1.5 rounded-full bg-[#18c7f4]" />{copy.eyebrow}</div><h1 className="text-3xl font-semibold tracking-[-.045em] sm:text-4xl">{copy.title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/35">{copy.subtitle}</p></div>
          <div className="flex items-center gap-2">{selectedSet && (isAdmin || selectedSet.access_level === "manage") && <button onClick={() => setChunkingOpen(true)} title={isFa ? "تنظیمات Chunking" : "Chunking settings"} className="app-icon-button grid size-11 place-items-center rounded-xl text-white/45 hover:text-[#d9a6ff]"><Settings2 size={16} /></button>}{selectedSetId && <button onClick={() => setPlaygroundOpen(true)} className="app-icon-button flex h-11 items-center gap-2 rounded-xl px-3 text-xs text-[#d9a6ff] sm:px-4"><FlaskConical size={16} /><span className="hidden sm:inline">{isFa ? "آزمایش بازیابی" : "Playground"}</span></button>}{!chatOpen && <button onClick={() => setChatOpen(true)} className="app-icon-button hidden h-11 items-center gap-2 rounded-xl px-4 text-sm text-white/65 sm:flex"><MessageSquareText size={17} />{copy.chatTitle}</button>}</div>
        </header>

        <section className="shrink-0">
          <div className="mb-3 flex items-center justify-between"><h2 className="text-xs font-semibold uppercase tracking-[.14em] text-white/35">{copy.sets}</h2>{isAdmin && <button onClick={() => setDialog("create")} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-semibold text-[#d9a6ff] hover:bg-[#7c27ff]/20"><Plus size={14} />{copy.newSet}</button>}</div>
          <div className="flex gap-3 overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-white/10">
            {sets.map((item) => <button key={item.id} onClick={() => setSelectedSetId(item.id)} className={`min-w-[210px] rounded-2xl border p-4 text-start transition ${selectedSetId === item.id ? "border-[#18c7f4]/45 bg-[#7c27ff]/25 shadow-[0_12px_35px_rgba(124,39,255,.2)]" : "border-white/[.07] bg-white/[.025] hover:bg-white/[.045]"}`}><div className="flex items-start justify-between"><span className={`grid size-9 place-items-center rounded-xl ${selectedSetId === item.id ? "bg-[#18c7f4]/20 text-[#d9a6ff]" : "bg-white/[.04] text-white/35"}`}><FolderKanban size={17} /></span>{selectedSetId === item.id && <span className="mt-1 size-1.5 rounded-full bg-[#c43cff] shadow-[0_0_10px_#c43cff]" />}</div><p className="mt-3 truncate text-sm font-semibold text-white/80">{item.name}</p><p className="mt-1 line-clamp-1 text-[11px] text-white/30">{item.description || (isFa ? "بدون توضیحات" : "No description")}</p><p className="mt-3 text-[10px] text-white/25">{item.document_count} {copy.documents} · {item.indexed_document_count} {copy.indexed}</p></button>)}
...
function CloudConnectorDialog(props: { setId: string; isFa: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  if (webhookInfo) return <div className="fixed inset-0 z-[85] grid place-items-center bg-black/75 p-4 backdrop-blur-md"><div className="app-glass-panel w-full max-w-xl rounded-[24px] border border-white/10 p-6"><span className="grid size-10 place-items-center rounded-xl bg-[#7c27ff]/30 text-[#d9a6ff]"><Zap size={18} /></span><h2 className="mt-4 text-xl font-semibold">{isFa ? "Webhook آماده است" : "Webhook is ready"}</h2><p className="mt-2 text-xs leading-5 text-amber-100/45">{isFa ? "Secret فقط همین یک‌بار نمایش داده می‌شود؛ اکنون آن را ذخیره کنید." : "The secret is shown only once. Store it securely now."}</p>{([['Endpoint', webhookInfo.endpoint], ['X-Webhook-Secret', webhookInfo.secret]] as const).map(([label, value]) => <div key={label} className="mt-4"><p className="mb-2 text-[10px] font-semibold text-white/40">{label}</p><div className="flex items-center gap-2 rounded-xl border border-white/[.08] bg-black/25 p-2"><code className="min-w-0 flex-1 overflow-x-auto px-2 text-[10px] text-white/60">{value}</code><button type="button" onClick={() => { void navigator.clipboard.writeText(value); toast.success("Copied"); }} className="rounded-lg bg-white/[.06] px-3 py-2 text-[10px] text-white/55">Copy</button></div></div>)}<button type="button" onClick={onSaved} className="mt-6 w-full rounded-xl bg-[#7c27ff] px-5 py-3 text-xs font-semibold">{isFa ? "انجام شد" : "Done"}</button></div></div>;
  return <div className="fixed inset-0 z-[85] grid place-items-center bg-black/75 p-4 backdrop-blur-md" onMouseDown={onClose}><motion.form initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} onSubmit={submit} onMouseDown={(event) => event.stopPropagation()} className="app-glass-panel max-h-[92dvh] w-full max-w-xl overflow-y-auto rounded-[24px] border border-white/10 p-6"><div className="flex items-start justify-between"><div><span className="grid size-10 place-items-center rounded-xl bg-[#7c27ff]/30 text-[#d9a6ff]"><Link2 size={18} /></span><h2 className="mt-4 text-xl font-semibold">{isFa ? "اتصال منبع جدید" : "Connect a new source"}</h2><p className="mt-2 text-xs text-white/30">{isFa ? "منبع ابری یا عمومی را به این پایگاه دانش متصل کنید." : "Connect a cloud or public source to this knowledge base."}</p></div><button type="button" onClick={onClose} className="app-icon-button grid size-9 place-items-center rounded-xl text-white/40"><X size={16} /></button></div><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-3">{options.map((option) => <button type="button" key={option.type} onClick={() => { setType(option.type); setUrl(""); }} className={`flex items-center gap-2 rounded-xl border p-3 text-xs transition ${type === option.type ? "border-[#18c7f4]/35 bg-[#7c27ff]/25 text-white/80" : "border-white/[.08] text-white/35 hover:bg-white/[.035]"}`}>{option.icon}{option.label}</button>)}</div><label className="mt-5 block text-xs font-semibold text-white/50">{isFa ? "نام اتصال" : "Connection name"}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={options.find((option) => option.type === type)?.label} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm outline-none placeholder:text-white/20 focus:border-[#18c7f4]/50" /></label><label className="mt-4 block text-xs font-semibold text-white/50">URL<input type="url" value={url} onChange={(event) => setUrl(event.target.value)} placeholder={placeholders[type]} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm outline-none placeholder:text-white/20 focus:border-[#18c7f4]/50" /></label><div className="mt-4 rounded-xl border border-white/[.07] bg-black/20 p-3 text-[10px] leading-5 text-white/35">{type === "google_drive" ? "Google OAuth credentials and a folder URL are required." : type === "s3" ? "Use an AWS S3 HTTPS bucket/prefix URL; credentials stay on the server." : type === "sharepoint" ? "Use the Microsoft Graph /children URL for the target drive folder." : "Only public HTTPS sources are supported."}</div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-xs text-white/40">{isFa ? "انصراف" : "Cancel"}</button><button disabled={!valid || saving} className="rounded-xl bg-[#7c27ff] px-5 py-2.5 text-xs font-semibold disabled:opacity-40">{saving ? (isFa ? "در حال همگام‌سازی…" : "Syncing…") : isFa ? "اتصال و همگام‌سازی" : "Connect and sync"}</button></div></motion.form></div>;
}
...
function ConnectorDialog({ setId, isFa, onClose, onSaved }: { setId: string; isFa: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-[85] grid place-items-center bg-black/75 p-4 backdrop-blur-md" onMouseDown={onClose}><motion.form initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} onSubmit={submit} onMouseDown={(e) => e.stopPropagation()} className="app-glass-panel w-full max-w-md rounded-[24px] border border-white/10 p-6"><div className="flex items-start justify-between"><div><span className="grid size-10 place-items-center rounded-xl bg-[#7c27ff]/30 text-[#d9a6ff]"><Link2 size={18} /></span><h2 className="mt-4 text-xl font-semibold">{isFa ? "اتصال منبع جدید" : "Connect a new source"}</h2><p className="mt-2 text-xs leading-5 text-white/30">{isFa ? "محتوای عمومی وب یا مخزن GitHub را به این مجموعه اضافه کنید." : "Import public web content or a GitHub repository into this set."}</p></div><button type="button" onClick={onClose} className="app-icon-button grid size-9 place-items-center rounded-xl text-white/40"><X size={16} /></button></div><div className="mt-6 grid grid-cols-2 gap-2">{(["website", "github"] as const).map((value) => <button type="button" key={value} onClick={() => setType(value)} className={`flex items-center gap-2 rounded-xl border p-3 text-xs ${type === value ? "border-[#18c7f4]/35 bg-[#7c27ff]/25 text-white/75" : "border-white/[.08] text-white/35"}`}>{value === "website" ? <Globe2 size={15} /> : <Github size={15} />}{value === "website" ? "Website URL" : "GitHub"}</button>)}</div><label className="mt-5 block text-xs font-semibold text-white/50">{isFa ? "نام اتصال" : "Connection name"}<input value={name} onChange={(e) => setName(e.target.value)} placeholder={type === "github" ? "Product repository" : "Documentation website"} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm outline-none placeholder:text-white/20 focus:border-[#18c7f4]/50" /></label><label className="mt-4 block text-xs font-semibold text-white/50">URL<input type="url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder={type === "github" ? "https://github.com/owner/repository" : "https://example.com/docs"} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm outline-none placeholder:text-white/20 focus:border-[#18c7f4]/50" /></label><div className="mt-4 rounded-xl border border-amber-300/10 bg-amber-300/[.04] p-3 text-[9px] leading-4 text-white/30">{isFa ? "فقط منابع عمومی HTTPS پشتیبانی می‌شوند. وب‌سایت یک صفحه و GitHub حداکثر ۴۰ فایل متنی را همگام می‌کند." : "Only public HTTPS sources are supported. Website sync imports one page; GitHub sync imports up to 40 supported text files."}</div><div className="mt-6 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-xs text-white/40">{isFa ? "انصراف" : "Cancel"}</button><button disabled={!valid || saving} className="rounded-xl bg-[#7c27ff] px-5 py-2.5 text-xs font-semibold disabled:opacity-40">{saving ? (isFa ? "در حال همگام‌سازی…" : "Syncing…") : isFa ? "اتصال و همگام‌سازی" : "Connect and sync"}</button></div></motion.form></div>;
}
...
function ChunkingSettingsDialog({ item, isFa, onClose, onSaved }: { item: DocumentSet; isFa: boolean; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-[90] grid place-items-center bg-black/75 p-4 backdrop-blur-md" onMouseDown={onClose}><motion.div initial={{ opacity: 0, scale: .97 }} animate={{ opacity: 1, scale: 1 }} onMouseDown={(event) => event.stopPropagation()} className="app-glass-panel w-full max-w-lg rounded-[26px] border border-white/10 p-6"><div className="flex items-start justify-between"><div><span className="grid size-10 place-items-center rounded-xl bg-[#7c27ff]/30 text-[#d9a6ff]"><Settings2 size={18} /></span><h2 className="mt-4 text-xl font-semibold">{isFa ? "تنظیمات Chunking" : "Chunking settings"}</h2><p className="mt-2 text-xs leading-5 text-white/30">{isFa ? `تنظیمات اختصاصی «${item.name}» برای اسناد جدید و پردازش مجدد.` : `Per-knowledge-base settings for new and reprocessed documents in “${item.name}”.`}</p></div><button onClick={onClose} className="app-icon-button grid size-9 place-items-center rounded-xl text-white/40"><X size={16} /></button></div><div className="mt-6 grid grid-cols-3 gap-2">{presets.map((preset) => <button key={preset.key} onClick={() => { setChild(preset.values[0]); setOverlap(preset.values[1]); setParent(preset.values[2]); }} className={`rounded-xl border p-3 text-[10px] font-semibold ${child === preset.values[0] && overlap === preset.values[1] && parent === preset.values[2] ? "border-[#18c7f4]/40 bg-[#7c27ff]/25 text-[#e6c7ff]" : "border-white/[.08] text-white/35"}`}>{preset.label}</button>)}</div><div className="mt-5 grid gap-4 sm:grid-cols-3"><ChunkNumber label={isFa ? "اندازه Child" : "Child size"} value={child} min={200} max={2000} onChange={setChild} /><ChunkNumber label={isFa ? "هم‌پوشانی" : "Overlap"} value={overlap} min={0} max={500} onChange={setOverlap} /><ChunkNumber label={isFa ? "اندازه Parent" : "Parent size"} value={parent} min={600} max={8000} onChange={setParent} /></div><div className="mt-5 rounded-xl border border-white/[.07] bg-black/20 p-3 text-[9px] leading-5 text-white/30">{isFa ? "Child کوچک‌تر بازیابی دقیق‌تری ایجاد می‌کند؛ Parent بزرگ‌تر زمینهٔ بیشتری به مدل می‌دهد. Overlap باید از Child کوچک‌تر باشد." : "Smaller children improve retrieval precision; larger parents provide more answer context. Overlap must remain smaller than the child size."}</div>{!valid && <p className="mt-3 text-[10px] text-rose-300/65">{isFa ? "مقادیر معتبر نیستند؛ Parent باید حداقل برابر Child و Overlap کمتر از Child باشد." : "Invalid values: parent must be at least child size and overlap must be smaller than child size."}</p>}<div className="mt-6 flex justify-end gap-2"><button onClick={onClose} className="rounded-xl px-4 py-2.5 text-xs text-white/40">{isFa ? "انصراف" : "Cancel"}</button><button disabled={!valid || saving} onClick={() => void save()} className="rounded-xl bg-[#7c27ff] px-5 py-2.5 text-xs font-semibold disabled:opacity-40">{saving ? "…" : isFa ? "ذخیره تنظیمات" : "Save settings"}</button></div></motion.div></div>;
}
...
function ChunkNumber({ label, value, min, max, onChange }: { label: string; value: number; min: number; max: number; onChange: (value: number) => void }) {
  return <label className="text-[10px] font-semibold text-white/45">{label}<input type="number" value={value} min={min} max={max} onChange={(event) => onChange(Number(event.target.value))} className="mt-2 h-11 w-full rounded-xl border border-white/[.09] bg-black/25 px-3 text-sm text-white/70 outline-none focus:border-[#18c7f4]/45" /><span className="mt-1 block text-[8px] font-normal text-white/20">{min} – {max} chars</span></label>;
}
