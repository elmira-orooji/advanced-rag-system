            ...
            {sets.map((item) => {
              ...
              return <article key={item.id} className="kb-set-card-wrap relative">
                <button onClick={() => { selectedSetIdRef.current = item.id; setSelectedSetId(item.id); }} aria-pressed={selectedSetId === item.id} className={`kb-set-card ${selectedSetId === item.id ? "is-active" : ""}`}>
                  ...
                  <p className="mt-3 truncate text-sm font-semibold kb-text">{item.name}</p><p className="mt-1 line-clamp-1 text-xs kb-muted">{item.description || (isFa ? "بدون توضیحات" : "No description")}</p><p className="mt-3 text-xs kb-muted">{item.document_count} {copy.documents} · {item.indexed_document_count} {copy.indexed}</p>
                </button>
