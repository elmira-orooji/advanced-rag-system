...
import { CheckCircle2, Copy, FileText, Quote, Share2, ShieldAlert, Sparkles, Telescope, ThumbsDown, ThumbsUp, X } from "lucide-react";
      ...
      {messages.map((message) => <article key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
        ...
        <div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[82%] ${message.role === "user" ? "rounded-br-md bg-[#7c27ff] text-white shadow-[0_10px_28px_rgba(124,39,255,.25)]" : "rounded-tl-md border border-white/[.08] bg-white/[.045] text-white/75"}`}>
          ...
          {message.role === "assistant" && <>
            <div className={`mt-3 flex items-center gap-1.5 text-[10px] ${message.grounded ? "text-emerald-200/50" : "text-amber-200/45"}`}>{message.answerBasis === "general" ? <><ShieldAlert size={12} />{isFa ? "دانش عمومی مدل؛ بدون استناد به اسناد" : "General model knowledge · not document-grounded"}</> : message.answerBasis === "hybrid" ? <><CheckCircle2 size={12} />{isFa ? "حالت ترکیبی؛ فقط بخش‌های ارجاع‌دار مستند هستند" : "Hybrid mode · only cited claims are source-linked"}</> : message.grounded ? <><CheckCircle2 size={12} />{isFa ? "پاسخ متصل به منبع" : "Source-linked answer"}</> : <><ShieldAlert size={12} />{isFa ? "بدون ارجاع به منبع" : "No source citation"}</>}</div>
            {message.research && <details className="mt-3 rounded-xl border border-[#18c7f4]/15 bg-[#7c27ff]/10 p-3"><summary className="flex cursor-pointer list-none items-center gap-2 text-[10px] font-semibold text-[#d9a6ff]"><Telescope size={12} />{isFa ? "مراحل پژوهش" : "Research trail"}<span className="ms-auto text-[9px] font-normal text-white/25">{message.research.steps.length} {isFa ? "جست‌وجو" : "searches"} · {message.research.evidenceReviewed} {isFa ? "شاهد" : "evidence"}</span></summary><div className="mt-3 space-y-2 border-t border-white/[.06] pt-3">{message.research.steps.map((step, index) => <div key={index} className="flex gap-2 text-[9px] leading-4 text-white/35"><span className="grid size-4 shrink-0 place-items-center rounded bg-[#7c27ff]/40 text-[8px] text-[#d9a6ff]">{index + 1}</span><span className="min-w-0 flex-1">{step.query}</span><span className="shrink-0 text-white/20">{step.evidence_count}</span></div>)}</div></details>}
            {message.sources?.length ? <div className="mt-3 border-t border-white/[.07] pt-3"><p className="mb-2 text-[10px] font-bold uppercase tracking-[.16em] text-[#c43cff]">{isFa ? "شواهد" : "Evidence"}</p><div className="flex flex-wrap gap-2">{message.sources.map((source) => <button key={`${source.id}-${source.citationId}`} onClick={() => setEvidence(source)} className="flex max-w-full items-center gap-1.5 rounded-lg border border-white/[.07] bg-black/15 px-2.5 py-1.5 text-[11px] text-white/45 transition hover:border-[#18c7f4]/30 hover:bg-[#7c27ff]/15 hover:text-white/70"><span className="grid size-4 shrink-0 place-items-center rounded bg-[#7c27ff]/40 text-[9px] text-[#e6c7ff]">{source.citationId}</span><FileText size={11} /><span className="truncate">{source.title}</span></button>)}</div></div> : null}
...
function ShareDialog({ messages, isFa, onClose }: { messages: ChatMessage[]; isFa: boolean; onClose: () => void }) {
  ...
  return <div className="absolute inset-0 z-40 grid place-items-center rounded-[20px] bg-black/75 p-3 backdrop-blur-md"><div className="w-full max-w-sm rounded-[22px] border border-white/10 bg-[rgba(18,14,25,.98)] p-5 shadow-2xl"><div className="flex items-center justify-between"><div><h3 className="text-sm font-semibold">{isFa ? "اشتراک‌گذاری گفتگو" : "Share conversation"}</h3><p className="mt-1 text-[9px] text-white/25">{isFa ? "یک نسخه فقط‌خواندنی منتشر می‌شود." : "A read-only snapshot will be published."}</p></div><button onClick={onClose} className="grid size-8 place-items-center rounded-lg text-white/30 hover:bg-white/5"><X size={14} /></button></div>{created ? <div className="mt-5"><div className="flex gap-2"><input readOnly value={url} className="h-10 min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-3 text-[10px] text-white/45" /><button onClick={() => { void navigator.clipboard.writeText(url); toast.success(isFa ? "لینک کپی شد" : "Link copied"); }} className="rounded-xl bg-[#7c27ff] px-4 text-[10px] font-semibold">{isFa ? "کپی" : "Copy"}</button></div><button onClick={() => void revoke()} className="mt-4 w-full rounded-xl border border-rose-300/10 py-2.5 text-[10px] text-rose-200/55">{isFa ? "لغو دسترسی" : "Revoke access"}</button></div> : <><div className="mt-5 grid grid-cols-2 gap-2">{(["team", "link"] as const).map((value) => <button key={value} onClick={() => setVisibility(value)} className={`rounded-xl border p-3 text-start ${visibility === value ? "border-[#18c7f4]/35 bg-[#7c27ff]/25" : "border-white/[.08]"}`}><p className="text-[11px] font-semibold text-white/65">{value === "team" ? (isFa ? "اعضای تیم" : "Team members") : (isFa ? "لینک عمومی" : "Public link")}</p><p className="mt-1 text-[9px] text-white/25">{value === "team" ? (isFa ? "نیازمند ورود" : "Sign-in required") : (isFa ? "هرکس لینک را دارد" : "Anyone with link")}</p></button>)}</div><div className="mt-4 flex gap-1 rounded-xl bg-black/20 p-1">{([1, 7, 30] as const).map((value) => <button key={value} onClick={() => setExpiry(value)} className={`flex-1 rounded-lg py-2 text-[9px] ${expiry === value ? "bg-[#7c27ff] text-white" : "text-white/30"}`}>{isFa ? `${value} روز` : `${value} days`}</button>)}</div><button disabled={saving} onClick={() => void create()} className="mt-5 h-10 w-full rounded-xl bg-[#7c27ff] text-[10px] font-semibold disabled:opacity-50">{saving ? "…" : isFa ? "ایجاد لینک امن" : "Create secure link"}</button></>}</div></div>;
}
...
function MessageFeedback({ responseId, isFa }: { responseId: string; isFa: boolean }) {
  ...
  return <div className="relative flex items-center gap-1">
    ...
    {open && <div className="nexora-dropdown absolute bottom-9 start-0 z-30 w-72 rounded-2xl border border-white/10 bg-[rgba(18,14,25,.98)] p-3 shadow-2xl backdrop-blur-2xl"><div className="flex items-center justify-between"><p className="text-[11px] font-semibold text-white/70">{isFa ? "مشکل پاسخ چه بود؟" : "What was wrong with the answer?"}</p><button onClick={() => setOpen(false)} className="grid size-6 place-items-center rounded-md text-white/30 hover:bg-white/5"><X size={12} /></button></div><div className="mt-2 space-y-1">{reasons.map(([value, label]) => <button key={value} aria-pressed={reason === value} onClick={() => setReason(value)} className={`flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-start text-[10px] ${reason === value ? "bg-[#7c27ff]/35 text-white/75" : "text-white/40 hover:bg-white/[.04]"}`}><span className={`size-2 rounded-full border ${reason === value ? "border-[#c43cff] bg-[#c43cff]" : "border-white/20"}`} />{label}</button>)}</div>{reason === "other" && <textarea value={comment} onChange={(e) => setComment(e.target.value)} maxLength={500} rows={2} placeholder={isFa ? "توضیح اختیاری..." : "Optional details..."} className="mt-2 w-full resize-none rounded-xl border border-white/10 bg-black/20 p-2.5 text-[10px] outline-none placeholder:text-white/20 focus:border-[#18c7f4]/40" />}<button disabled={!reason || saving} onClick={() => void negative()} className="mt-3 h-9 w-full rounded-xl bg-[#7c27ff] text-[10px] font-semibold disabled:opacity-40">{saving ? "…" : isFa ? "ثبت بازخورد" : "Submit feedback"}</button></div>}
  </div>;
}
