      ...
      {messages.map((message) => <article key={message.id} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}>
        ...
        <div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[82%] ${message.role === "user" ? "rounded-br-md bg-[#32127A] text-white shadow-[0_10px_28px_rgba(50,18,122,.25)]" : "rounded-tl-md border border-white/[.08] bg-white/[.045] text-white/75"}`}>
          ...
          {message.role === "assistant" && <>
            ...
            {message.research && <details className="mt-3 rounded-xl border border-[#8f78d8]/15 bg-[#32127A]/10 p-3"><summary className="flex cursor-pointer list-none items-center gap-2 text-[10px] font-semibold text-[#b6a7ef]"><Telescope size={12} />{isFa ? "مراحل پژوهش" : "Research trail"}<span className="ms-auto text-[9px] font-normal text-white/25">{message.research.steps.length} {isFa ? "جست‌وجو" : "searches"} · {message.research.evidenceReviewed} {isFa ? "شاهد" : "evidence"}</span></summary><div className="mt-3 space-y-2 border-t border-white/[.06] pt-3">{message.research.steps.map((step, index) => <div key={index} className="flex gap-2 text-[9px] leading-4 text-white/35"><span className="grid size-4 shrink-0 place-items-center rounded bg-[#32127A]/40 text-[8px] text-[#b6a7ef]">{index + 1}</span><span className="min-w-0 flex-1">{step.query}</span><span className="shrink-0 text-white/20">{step.evidence_count}</span></div>)}</div></details>}
