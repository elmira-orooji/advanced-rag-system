...
export default function DocumentChunkInspector({ documentId, isAdmin, isFa, onClose }: Props) {
  ...
  return <div className="fixed inset-0 z-[96] flex justify-end bg-black/70 backdrop-blur-sm" onMouseDown={onClose}><aside onMouseDown={(event) => event.stopPropagation()} className="flex h-full w-full max-w-5xl flex-col border-s border-white/10 bg-[rgba(9,7,13,.98)] shadow-2xl">
    <header className="flex h-20 shrink-0 items-center justify-between border-b border-white/[.08] px-5"><div className="flex min-w-0 items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#7c27ff]/30 text-[#d9a6ff]"><Boxes size={18} /></span><div className="min-w-0"><h2 className="truncate text-sm font-semibold">{isFa ? "بازرس سند و قطعه‌ها" : "Document / Chunk Inspector"}</h2><p className="mt-1 truncate text-[10px] text-white/30">{document?.filename || (isFa ? "در حال بارگذاری…" : "Loading…")}</p></div></div><button onClick={onClose} className="app-icon-button grid size-9 place-items-center rounded-xl"><X size={16} /></button></header>
    {loading ? <div className="grid flex-1 place-items-center"><span className="size-5 animate-spin rounded-full border-2 border-white/10 border-t-[#c43cff]" /></div> : document && <>
...
function ChunkEnrichment({ chunk, isFa, isAdmin, saving, onEnrich }: { chunk: DocumentChunk; isFa: boolean; isAdmin: boolean; saving: boolean; onEnrich: () => void }) {
  ...
  return <div className="max-h-48 shrink-0 overflow-y-auto border-b border-white/[.07] bg-white/[.012] px-4 py-3"><div className="flex items-center justify-between"><p className="flex items-center gap-2 text-[10px] font-semibold text-white/45"><Sparkles size={12} className="text-[#c43cff]" />{isFa ? "درک معنایی Chunk" : "Chunk intelligence"}</p>{isAdmin && <button disabled={saving} onClick={onEnrich} className="rounded-lg border border-[#18c7f4]/15 px-2.5 py-1.5 text-[9px] text-[#d9a6ff] disabled:opacity-40">{empty ? (isFa ? "تولید" : "Generate") : (isFa ? "تولید مجدد" : "Regenerate")}</button>}</div>{empty ? <p className="mt-3 text-[9px] text-white/20">{isFa ? "برای این Chunk هنوز Keyword یا سؤال پیشنهادی تولید نشده است." : "No keywords or suggested questions have been generated yet."}</p> : <div className="mt-3 grid gap-3 md:grid-cols-2"><div><p className="mb-2 flex items-center gap-1.5 text-[9px] text-white/25"><Tags size={10} />Keywords</p><div className="flex flex-wrap gap-1.5">{chunk.keywords.map((keyword) => <span key={keyword} className="rounded-lg border border-[#18c7f4]/15 bg-[#7c27ff]/15 px-2 py-1 text-[9px] text-[#e6c7ff]/75">{keyword}</span>)}</div></div><div><p className="mb-2 text-[9px] text-white/25">{isFa ? "سؤال‌های پیشنهادی" : "Suggested questions"}</p><ul className="space-y-1.5">{chunk.suggested_questions.map((question) => <li key={question} className="text-[9px] leading-4 text-white/40">• {question}</li>)}</ul></div></div>}</div>;
}

function HighlightedSource({ text, query }: { text: string; query: string }) {
  const needle = query.trim();
  if (!needle) return <mark className="mt-4 block whitespace-pre-wrap rounded-xl bg-[#7c27ff]/25 p-3 text-[11px] leading-6 text-white/65">{text}</mark>;
  const parts = text.split(new RegExp(`(${needle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})`, "gi"));
  return <p className="mt-4 whitespace-pre-wrap rounded-xl bg-white/[.025] p-3 text-[11px] leading-6 text-white/55">{parts.map((part, index) => part.toLocaleLowerCase() === needle.toLocaleLowerCase() ? <mark key={index} className="rounded bg-[#7c27ff]/55 px-0.5 text-white">{part}</mark> : part)}</p>;
}
