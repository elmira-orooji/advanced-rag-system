...
export default function DocumentChunkInspector({ documentId, isAdmin, isFa, onClose }: Props) {
  ...
  return createPortal(<div className="app-shell"><div className="kb-page inspector-overlay" style={{ background: "#18213380", fontFamily: isFa ? "Vazirmatn, sans-serif" : "Inter, sans-serif" }} dir={isFa ? "rtl" : "ltr"} onMouseDown={onClose}><aside role="dialog" aria-modal="true" aria-labelledby="inspector-title" onKeyDown={(event) => { if (event.key === "Escape" && !editing) onClose(); }} onMouseDown={(event) => event.stopPropagation()} className="inspector-panel">
    <header className="inspector-header flex h-20 shrink-0 items-center justify-between border-b inspector-border px-5"><div className="flex min-w-0 items-center gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl inspector-tint inspector-accent"><Boxes size={18} /></span><div className="min-w-0"><h2 id="inspector-title" className="truncate text-sm font-semibold">{isFa ? "بازرس سند و قطعه‌ها" : "Document / Chunk Inspector"}</h2><p className="mt-1 truncate text-[10px] inspector-muted">{document?.filename || (isFa ? "در حال بارگذاری…" : "Loading…")}</p></div></div><button aria-label={isFa ? "بستن بازرس" : "Close inspector"} onClick={onClose} className="app-icon-button grid size-9 place-items-center rounded-xl"><X size={16} /></button></header>
    ...
    {loading ? <div className="grid flex-1 place-items-center"><span className="size-5 animate-spin rounded-full border-2 inspector-border border-t-[#c43cff]" /></div> : document && <>
      ...
      {previewOpen && previewUrl && <div className="grid h-[44%] min-h-[260px] shrink-0 border-b inspector-border inspector-subtle md:grid-cols-[1fr_310px]"><iframe title={document.filename} src={`${previewUrl}#page=${selectedChunk?.page_number || 1}&view=FitH`} className="h-full w-full bg-white" /><aside className="min-h-0 overflow-y-auto border-s inspector-border p-4"><div className="flex items-center justify-between"><p className="text-[10px] font-semibold inspector-accent">{isFa ? "منبع انتخاب‌شده" : "Selected source"}</p>{selectedChunk?.page_number && <span className="rounded-lg inspector-tint px-2 py-1 text-[9px] inspector-accent">{isFa ? "صفحه" : "Page"} {selectedChunk.page_number}</span>}</div>{selectedChunk ? <HighlightedSource text={selectedChunk.content} query={query} /> : <p className="mt-5 text-[10px] leading-5 inspector-muted">{isFa ? "یک Child Chunk را انتخاب کنید تا منبع آن برجسته شود." : "Select a child chunk to highlight its source."}</p>}</aside></div>}
      {selectedChunk && <ChunkEnrichment chunk={selectedChunk} isFa={isFa} isAdmin={isAdmin} saving={saving} onEnrich={() => void enrichSelected()} />}
      {isAdmin && selectedChunk && <div className="shrink-0 border-b inspector-border inspector-tint p-3">{editing ? <div><textarea autoFocus value={draft} onChange={(event) => setDraft(event.target.value)} className="max-h-40 min-h-24 w-full resize-y rounded-xl border inspector-border inspector-subtle p-3 text-xs leading-5 inspector-text outline-none focus:inspector-border" /><div className="mt-2 flex justify-end gap-2"><button onClick={() => setEditing(false)} className="rounded-lg px-3 py-2 text-[10px] inspector-muted">{isFa ? "انصراف" : "Cancel"}</button><button disabled={saving || draft.trim().length < 20} onClick={() => void updateSelected({ content: draft.trim() })} className="rounded-lg bg-[#7c27ff] px-4 py-2 text-[10px] font-semibold disabled:opacity-40">{isFa ? "ذخیره تغییرات" : "Save changes"}</button></div></div> : <div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-semibold inspector-text">CHILD {selectedChunk.chunk_index + 1}</p><p className={`mt-1 text-[9px] ${selectedChunk.is_active ? "text-emerald-300/55" : "text-amber-300/60"}`}>{selectedChunk.is_active ? (isFa ? "فعال در Retrieval" : "Active in retrieval") : (isFa ? "غیرفعال و خارج از Retrieval" : "Disabled from retrieval")}</p></div><div className="flex gap-2"><button onClick={() => { setDraft(selectedChunk.content); setEditing(true); }} className="flex h-9 items-center gap-1.5 rounded-lg border inspector-border px-3 text-[10px] inspector-muted"><Pencil size={12} />{isFa ? "ویرایش" : "Edit"}</button><button disabled={saving} onClick={() => void updateSelected({ is_active: !selectedChunk.is_active })} className={`flex h-9 items-center gap-1.5 rounded-lg border px-3 text-[10px] ${selectedChunk.is_active ? "border-amber-300/15 text-amber-200/60" : "border-emerald-300/15 text-emerald-200/60"}`}><Power size={12} />{selectedChunk.is_active ? (isFa ? "غیرفعال‌کردن" : "Disable") : (isFa ? "فعال‌کردن" : "Enable")}</button></div></div>}</div>}
      <div className="inspector-workspace"><nav aria-label={isFa ? "بخش‌های والد" : "Parent blocks"} className="inspector-nav min-h-0 overflow-y-auto border-e inspector-border p-3">{parents.map((item) => <button key={item.index} aria-pressed={parent?.index === item.index} onClick={() => { setParentIndex(item.index); setSelectedChunk(null); setEditing(false); }} className={`mb-1 flex w-full items-center gap-3 rounded-xl p-3 text-start ${parent?.index === item.index ? "inspector-tint inspector-text" : "inspector-muted hover:inspector-surface"}`}><span className="grid size-8 shrink-0 place-items-center rounded-lg inspector-subtle text-[10px] font-semibold">P{item.index + 1}</span><span className="min-w-0 flex-1"><span className="block truncate text-[11px] font-semibold">{isFa ? "بخش والد" : "Parent block"} {item.index + 1}</span><span className="mt-1 block text-[9px] inspector-muted">{item.children.length} child · {item.content.length} chars</span></span><ChevronRight size={12} /></button>)}</nav>
        <section className="inspector-content min-h-0 overflow-y-auto p-4">{!parent && <p className="inspector-empty">{isFa ? "قطعه‌ای با این جست‌وجو پیدا نشد." : "No chunks match your search."}</p>}{parent && <><div className="rounded-2xl border inspector-border inspector-tint p-4"><div className="mb-3 flex justify-between"><span className="text-[10px] font-semibold inspector-accent">PARENT {parent.index + 1}</span><span className="text-[9px] inspector-muted">{parent.content.length} chars</span></div><p className="whitespace-pre-wrap text-xs leading-6 inspector-text">{parent.content}</p></div><p className="my-4 text-[9px] font-semibold uppercase tracking-[.14em] inspector-muted">{isFa ? "قطعه‌های فرزند" : "Child chunks"}</p><div className="space-y-3">{parent.children.map((chunk) => <button key={chunk.id} aria-pressed={selectedChunk?.id === chunk.id} onClick={() => { chooseChunk(chunk); setEditing(false); }} className={`block w-full rounded-2xl border p-4 text-start transition ${selectedChunk?.id === chunk.id ? "inspector-border inspector-tint" : "inspector-border inspector-surface hover:inspector-surface"}`}><div className="mb-3 flex justify-between gap-2"><span className="flex items-center gap-2 text-[10px] font-semibold inspector-text"><FileText size={13} className="inspector-accent" />CHILD {chunk.chunk_index + 1}</span><span className="text-[9px] inspector-muted">{chunk.page_number ? `${isFa ? "صفحه" : "Page"} ${chunk.page_number} · ` : ""}{chunk.content.length} chars</span></div><p className="whitespace-pre-wrap text-xs leading-6 inspector-muted">{chunk.content}</p></button>)}</div></>}</section>
      </div>
    </>}
...
function ChunkEnrichment({ chunk, isFa, isAdmin, saving, onEnrich }: { chunk: DocumentChunk; isFa: boolean; isAdmin: boolean; saving: boolean; onEnrich: () => void }) {
  ...
  return <div className="max-h-48 shrink-0 overflow-y-auto border-b inspector-border inspector-surface px-4 py-3"><div className="flex items-center justify-between"><p className="flex items-center gap-2 text-[10px] font-semibold inspector-muted"><Sparkles size={12} className="inspector-accent" />{isFa ? "درک معنایی Chunk" : "Chunk intelligence"}</p>{isAdmin && <button disabled={saving} onClick={onEnrich} className="rounded-lg border inspector-border px-2.5 py-1.5 text-[9px] inspector-accent disabled:opacity-40">{empty ? (isFa ? "تولید" : "Generate") : (isFa ? "تولید مجدد" : "Regenerate")}</button>}</div>{empty ? <p className="mt-3 text-[9px] inspector-muted">{isFa ? "برای این Chunk هنوز Keyword یا سؤال پیشنهادی تولید نشده است." : "No keywords or suggested questions have been generated yet."}</p> : <div className="mt-3 grid gap-3 md:grid-cols-2"><div><p className="mb-2 flex items-center gap-1.5 text-[9px] inspector-muted"><Tags size={10} />Keywords</p><div className="flex flex-wrap gap-1.5">{chunk.keywords.map((keyword) => <span key={keyword} className="rounded-lg border inspector-border inspector-tint px-2 py-1 text-[9px] inspector-accent">{keyword}</span>)}</div></div><div><p className="mb-2 text-[9px] inspector-muted">{isFa ? "سؤال‌های پیشنهادی" : "Suggested questions"}</p><ul className="space-y-1.5">{chunk.suggested_questions.map((question) => <li key={question} className="text-[9px] leading-4 inspector-muted">• {question}</li>)}</ul></div></div>}</div>;
}

function HighlightedSource({ text, query }: { text: string; query: string }) {
  const needle = query.trim();
  if (!needle) return <mark className="mt-4 block whitespace-pre-wrap rounded-xl inspector-tint p-3 text-[11px] leading-6 inspector-text">{text}</mark>;
  const parts = text.split(new RegExp(`(${needle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")})`, "gi"));
  return <p className="mt-4 whitespace-pre-wrap rounded-xl inspector-surface p-3 text-[11px] leading-6 inspector-text">{parts.map((part, index) => part.toLocaleLowerCase() === needle.toLocaleLowerCase() ? <mark key={index} className="rounded inspector-tint px-0.5 text-white">{part}</mark> : part)}</p>;
}
